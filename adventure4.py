...
Game.player_inventory = Bag()


Room("bedroom", """This is the bedroom, there is a burning candle on the wall. There is also a comfortable bed.""")

Room("livingroom", """This is the livingroom. There is a sofa""")

Room("computerroom", """This is a room with computers, the computer room.""")

Room("kitchen", """This is the kitchen.""")

Room("garden", """ This is the garden, there are pretty and aromatic flowers here that you have planted. """)

Game.rooms["bedroom"].inventory = Bag()
Game.rooms["livingroom"].inventory = Bag()
Game.rooms["kitchen"].inventory = Bag()
Game.rooms["computerroom"].inventory = Bag()
Game.rooms["garden"].inventory = Bag()

Game.rooms["bedroom"].west = Game.rooms["livingroom"]
Game.rooms["livingroom"].north = Game.rooms["computerroom"]
Game.rooms["livingroom"].west = Game.rooms["garden"]
Game.rooms["computerroom"].east = Game.rooms["kitchen"]

Game.rooms["computerroom"].locked = {"east":True}
Game.rooms["bedroom"].locked = dict()
Game.rooms["kitchen"].locked = dict()
Game.rooms["livingroom"].locked = {"west":True}

Game.current_room = Game.rooms["bedroom"]
...
kitchen_key = Item("Brass key","There is an old rusty brass key on the floor. Try to find out what this is for.", )
kitchen_key.use_item = use_kitchen_key

Game.rooms["bedroom"].inventory.add(kitchen_key)

garden_key = Item("Iron key","On the kitchen counter there is a iron key. What could this be for?")
garden_key.use_item = use_garden_key

Game.rooms["kitchen"].inventory.add(garden_key)

fresh_baguettes = Item("Baguettes","There is a bag of freshly baked baguettes on the kitchen counter.")

Game.rooms["kitchen"].inventory.add(fresh_baguettes)
...
# ------------------------- GUI ----------------------------
layout_inventory = [
    [sg.Image(background_color = "white", key="image2")],
    [sg.Text("your items:")],
...
layout = [
        ...
        [sg.Output(size=(20,15),key="output1", ),
        
         sg.Column(layout_inventory),],
        
        
        [sg.Text("Location:"), sg.Text("Bedroom", key="location"), 
         sg.Text("directions:"),sg.Text("??????????:", key="directions")],
        [sg.Multiline("", size=(20,3),key="multi1", autoscroll=True)],
        [sg.Text("Your command: >>>", key="prompt", size=(20,1)),
         sg.Input(key="command", size=(10,1)), sg.Button("OK"),],
        [sg.Button("Quit")],
    ]
...
window["directions"].update(Game.current_room.exits())
while True:
    ...
    if event == "OK":
        window["multi1"].update(f"{values['multi1']}\nturn {Game.turn}: "+values["command"]+"\n")
        print(f"-----turn {Game.turn}-------")
        _handle_command(values["command"])
        Game.turn += 1
        # inventory update
        window["listbox1"].update(values=Game.player_inventory)
        ...
        window["listbox2"].update(values=Game.current_room.inventory)
        # ---- location update ----
        window["location"].update(Game.current_room.name)
        # ----- directions update ----
        window["directions"].update(Game.current_room.exits())
